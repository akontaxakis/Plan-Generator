...
public class TxtToAG_AG_EQ_experiments_Senario_1 {
    ...
    public static void main(String[] args) {
        ...
        for(int number_of_requests=1;number_of_requests<number_of_total_requests+1;number_of_requests++) {
            ...
            for (int i = 0; i < number_of_experiments; i++) {
                ...
                Proposition_plan best_c_Plan = AG.expand_closest_proposition(ag_rq);
                ...
                Proposition_plan best_c_Plan_2 = AG_2.expand_closest_proposition(ag2_rq);
                tmp = best_c_Plan_2;
                ...
                if ((best_c_Plan.getCost() > tmp.getCost() && tmp.getCost()>0 ) || tmp.getCost() == -1) {
                    ...
                    eq_total_latency =eq_total_latency +  AG.getLatency(ag_rq);;
                    lt_total_latency =lt_total_latency+ AG_2.getLatency(ag2_rq);
    ...
    private static ArrayList<Artifact> remove_gpu_artifact(ArrayList<Artifact> ag2_rq) {
        ArrayList<Artifact> rq = new ArrayList<>(ag2_rq);
        for(Artifact a:ag2_rq){
            if(a.getId().contains("GPU"))
                rq.remove(a);
        }
        return rq;
    }
